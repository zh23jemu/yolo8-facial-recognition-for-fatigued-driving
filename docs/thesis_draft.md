...
本文共分为七章。第 1 章介绍研究背景、研究意义、研究现状和主要研究内容。第 2 章介绍系统涉及的关键技术，重点分析 YOLOv8 检测机制、CBAM 注意力机制、训练评估指标和模型部署技术。第 3 章围绕本文核心算法，介绍基于注意力机制的 YOLOv8 面部识别模型设计，包括基线模型、CBAM 嵌入位置、特征增强逻辑和疲劳特征输出方式。第 4 章进行系统分析与总体设计，说明系统需求、数据集设计、总体流程和疲劳状态判定方案。第 5 章介绍系统核心模块设计与实现，包括模型训练、推理检测、桌面可视化和日志截图模块。第 6 章进行系统测试与性能分析，通过基线模型和注意力改进模型对比验证算法改进效果，并结合系统功能测试说明模型落地闭环。第 7 章总结全文工作，并提出后续改进方向。
...
从使用场景看，系统主要面向两类用户。一类是研究和开发人员，他们需要通过训练脚本、配置文件和实验结果复现模型训练过程；另一类是答辩或演示场景下的普通使用者，他们更关注界面是否直观、检测结果是否清楚、截图和日志是否容易保存。因此，本文在需求分析中兼顾算法实验与系统演示两种需求。训练侧强调参数可配置、结果可追踪和服务器适配；演示侧强调操作步骤少、反馈及时和异常提示明确。
...
在模型改进方案选择上，本文没有直接采用更大的 YOLOv8s、YOLOv8m 等模型，而是在 YOLOv8n 轻量骨干上加入注意力模块。这一设计考虑到疲劳驾驶检测通常需要实时运行，若盲目增加模型规模，虽然可能提升精度，但会增加推理延迟和部署成本。CBAM 模块参数量相对较小，加入后模型参数从约 3.01M 增至约 3.08M，整体规模变化不大，能够较好保持实时性。该设计也便于在论文实验中单独分析注意力机制对检测效果的影响。
...
疲劳状态判定并不是单纯的目标检测问题。检测模型只能告诉系统某一帧中是否出现闭眼或打哈欠目标，而疲劳通常表现为一段时间内状态持续异常。例如，正常眨眼时间很短，若仅凭单帧闭眼就触发疲劳报警，容易造成误报；而连续多帧闭眼或短时间内频繁打哈欠，则更接近真实疲劳表现。因此，本文在规则设计中采用滑动窗口思想，把最近若干帧的检测结果作为状态判断依据。

在当前实现中，规则模块重点统计闭眼特征和打哈欠特征。若窗口内闭眼比例超过较低阈值，系统输出疑似疲劳；若闭眼比例继续升高或打哈欠次数明显增加，系统输出疲劳。该方法虽然简单，但具有较好的可解释性，用户能够从界面检测框和日志记录中看到触发状态变化的原因。对于毕业设计系统演示而言，这种规则化设计便于验证，也便于在论文中说明算法链路。

为了与开题报告中的技术路线保持一致，项目同时实现了 LSTM + Attention 疲劳时序分类模型骨架。该模型预留了连续帧特征输入接口，输入可以由每帧 Eyeclosed、Eyeopen、Yawn 的置信度、出现次数或比例构成。LSTM 用于学习连续帧之间的时间依赖关系，Attention 层用于突出对最终状态判断贡献较大的关键帧。由于本文当前数据主要是图像检测数据，尚未形成足够完整的视频序列标签，因此时序模型在本阶段作为后续扩展基础保留，论文实验重点仍放在 YOLOv8n 与 YOLOv8n + CBAM 的检测性能对比上。
...
考虑到毕业设计答辩时演示环境可能不稳定，系统界面还对异常情况进行了提示处理。例如，当摄像头不可用、模型权重不存在或输入文件无法打开时，系统不会直接退出，而是弹出错误提示并保留主窗口可操作状态。图片输入默认只载入画面，用户点击“开始检测”后才执行模型推理，避免误以为选择文件后系统已经完成检测。开始检测后界面会显示正在检测相关提示，使用户能够明确系统当前处于模型加载或推理阶段。
...
训练过程中，服务器端首先验证 CUDA 是否可用，并记录 GPU 型号、PyTorch 版本和 Ultralytics 版本。基线模型使用 `yolov8n.pt` 作为预训练权重，CBAM 模型使用自定义 YAML 结构并从 YOLOv8n 预训练权重迁移初始化。为了保证两组实验具有可比性，除模型结构不同外，两组实验保持相同训练轮数、输入尺寸、batch size、优化器和数据增强策略。
...
![图 6-1 YOLOv8n 基线模型 PR 曲线](../runs/yolo/fatigue_yolov8n_slurm/BoxPR_curve.png)

图 6-1 展示了 YOLOv8n 基线模型在验证集上的 PR 曲线。PR 曲线能够反映不同置信度阈值下准确率与召回率之间的变化关系。当曲线整体更靠近右上角时，说明模型在保持较高召回率的同时仍能获得较高准确率。基线模型在闭眼类别上表现较好，但打哈欠类别仍存在一定提升空间。

![图 6-2 YOLOv8n + CBAM 模型 PR 曲线](../runs/yolo/fatigue_yolov8n_cbam/BoxPR_curve.png)

图 6-2 展示了引入 CBAM 后的 PR 曲线。对比基线模型可以看出，注意力机制对整体检测性能有一定提升，尤其在打哈欠类别上改善更明显。这与疲劳驾驶场景中嘴部区域属于局部关键特征的特点相吻合。

![图 6-3 YOLOv8n + CBAM 模型混淆矩阵](../runs/yolo/fatigue_yolov8n_cbam/confusion_matrix.png)

图 6-3 为 CBAM 模型的混淆矩阵。混淆矩阵能够直观反映不同类别之间的误检与漏检情况。疲劳驾驶面部识别中，Eyeclosed 与 Eyeopen 均位于眼部区域，二者在部分低分辨率或光照不足图像中可能出现混淆；Yawn 类别则主要受嘴部张开程度、头部姿态和遮挡影响。通过混淆矩阵分析，可以为后续数据清洗和样本增强提供依据。
...
![图 6-4 基线模型 F1 曲线](../runs/yolo/fatigue_yolov8n_slurm/BoxF1_curve.png)

![图 6-5 CBAM 模型 F1 曲线](../runs/yolo/fatigue_yolov8n_cbam/BoxF1_curve.png)

图 6-4 和图 6-5 分别展示了基线模型与 CBAM 模型的 F1 曲线。F1 值综合考虑 Precision 和 Recall，适合用于观察模型在不同置信度阈值下的综合性能。结合 PR 曲线、F1 曲线和最终验证指标可以看出，CBAM 模型在整体性能上优于基线模型，但提升幅度并非无限扩大，说明注意力机制能够增强关键局部特征，但最终效果仍受到数据规模、标签质量和类别分布影响。
...
![图 6-6 单张图片检测效果](images/test_predict.jpg)

图 6-6 为命令行单张图片推理结果。系统能够在图片中绘制检测框，并输出类别名称、置信度和疲劳状态。该测试验证了模型权重在本地环境下能够正常加载和推理。

![图 6-7 桌面系统打哈欠检测界面](images/system_demo_yawn.jpg)

图 6-7 为 PyQt5 桌面系统检测打哈欠状态的界面截图。界面能够显示检测框、类别置信度、疲劳状态和日志路径，说明系统演示链路已经打通。

![图 6-8 桌面系统检测结果示例](images/system_demo_second.jpg)

图 6-8 为另一张桌面系统检测截图。通过系统截图可以看出，本文系统不仅能够完成模型推理，还能够将结果以图形界面方式展示给用户，便于答辩演示和后续论文材料整理。
...
从指标变化看，CBAM 模型相对基线模型的提升主要体现在 Recall、mAP50 和 mAP50-95 上。Recall 的提升说明模型能够检出更多真实疲劳相关目标，mAP50 和 mAP50-95 的提升说明检测框定位和类别判断的综合性能有所改善。对于疲劳驾驶预警系统而言，漏检疲劳特征可能带来更高安全风险，因此 Recall 的提升具有实际意义。不过，CBAM 模型 Precision 与基线模型相比变化不大，说明模型在减少误检方面仍有进一步优化空间。

从类别结果看，Eyeclosed 类别表现最稳定，原因可能是闭眼样本数量较多，且闭眼特征边界相对清晰。Yawn 类别的 Recall 较高但 Precision 相对较低，说明模型倾向于积极检出打哈欠目标，但可能把部分张嘴、说话或嘴部遮挡样本误判为打哈欠。Eyeopen 类别受到眼部目标小、姿态变化和光照变化影响，指标也存在一定波动。针对这些问题，后续可以在数据层面增加复杂场景样本，在训练层面调整类别权重和增强策略，在推理层面结合连续帧规则减少单帧误报。

从系统测试看，当前桌面系统已经能够支撑论文实验展示。命令行推理可以用于快速验证权重是否可用，桌面界面可以用于展示用户交互和检测效果，日志与截图功能可以为论文第 6 章提供材料。由于本机摄像头条件受限，摄像头测试仍需要在后续具备硬件条件时进一步补充；但图片检测、模型加载、状态显示、截图保存和日志记录均已完成，能够满足当前毕业设计阶段的主要演示需求。
